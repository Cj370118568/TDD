...
* UI tests
	...
	#####小结：
	两个小demo分别展示了手动编写UI tests代码以及自动生成UI tests代码。可以看到，由于UI tests需要实际运行并渲染界面以模拟用户使用场景，测试时间长，违反了上面一条[原则](##TDD原则)，所以也有争论说不要对UI进行测试。
...
* 开始就进行TDD
	+ 项目介绍
		...
		题目页面：
		![题目](./resources/convertintro.jpg)
		
		分数页面：
		![UI-tests](./resources/convertintro1.jpg)
	+ v1.0
	
		产品需求：每人回答3题，范围由1～10
		<p><input type="checkbox" check/>演示：v1.0</p> 
		<br></br>
* 开发过程中加入TDD
* 在老项目中进行TDD
